...
public class CRPREP_ORDEN_INSTALACION_EMITIDA_RES_4_MEGAS {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT TRIM(MAX(ORDENNRO)) FROM ORDENSRV ORD WHERE ORDENSTS='E' AND ORDENTPO='I' AND CONTRATONRO IN (SELECT CONTRATONRO FROM CONTRATO CO WHERE ORD.EMPRESAID=CO.EMPRESAID AND CO.CONTRATONRO=ORD.CONTRATONRO AND PRODUCTOID IN (SELECT PRODUCTOID FROM PRODUCTO PO WHERE PRODUCTONOMBRE='INTERNET RESIDENCIAL 4 MEGAS' AND CO.PRODUCTOID=PO.PRODUCTOID AND PO.EMPRESAID=CO.EMPRESAID))"
