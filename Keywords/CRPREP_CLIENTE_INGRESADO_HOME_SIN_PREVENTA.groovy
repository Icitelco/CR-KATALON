...
public class CRPREP_CLIENTE_INGRESADO_HOME_SIN_PREVENTA {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT MAX(CLIENTENRO) FROM SIGATG.CLIENTE CL WHERE CLIENTESTS='I' AND NEGOCIOSEGMENTO=1 AND CLIENTENRO NOT IN (SELECT CLIENTENROPREVENTA FROM PREVENTACLIENTE PR WHERE CL.CLIENTENRO=PR.CLIENTENROPREVENTA)"
