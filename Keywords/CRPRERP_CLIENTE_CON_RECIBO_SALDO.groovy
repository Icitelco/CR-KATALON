...
public class CRPRERP_CLIENTE_CON_RECIBO_SALDO {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT MAX (CLIENTENRO) FROM RECIBO RE WHERE RECIBOTPO='R' AND RECIBOSDO<>0 AND RECIBOSTS='P' AND RECIBONRO NOT IN (SELECT RECIBONRORBO FROM RECIBO RE2 WHERE RECIBOTPO='P' AND RE.CLIENTENRO=RE2.CLIENTENRO AND RE.EMPRESAID=RE2.EMPRESAID)"
