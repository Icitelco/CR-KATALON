...
public class CRPREP_CLIENTE_CON_2017_SERVICIO_DIGITAL {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT TRIM(MAX(CLIENTENRO)) FROM CONTRATO CO WHERE CO.PRODUCTOID=3007 AND CO.CONTRATOSTS='C' AND CO.CLIENTENRO IN (SELECT PC.CLIENTENRO FROM PLANCOMERCIALCLIENTE PC WHERE PC.PLANCOMERCIALID=1259 ) AND CO.CLIENTENRO NOT IN (SELECT CS.CLIENTENRO FROM CLIENTESERVICIO CS WHERE CS.CLIENTESRVSTS IN ('I','P'))"
