...
public class CRPREP_CLIENTE_CON_PRODUCTO_DE_TEMPORAL {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT MAX(CLIENTENRO) FROM SIGATG.CLIENTE WHERE CLIENTESTS='C' AND CLIENTENRO IN (SELECT CLIENTENRO FROM SIGATG.CONTRATO WHERE CONTRATOSTS='C' AND PRODUCTOID IN (SELECT PRODUCTOID FROM SIGATG.PRODUCTOPRODUCTOTEMPORAL WHERE PRODUCTOPRDTEMPORALID IN (SELECT MAX(PRODUCTOTEMPORALID) FROM SIGATG.PRODUCTOTEMPORAL)))"
