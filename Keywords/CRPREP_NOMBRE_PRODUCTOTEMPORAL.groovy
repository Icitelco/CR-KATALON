...
public class CRPREP_NOMBRE_PRODUCTOTEMPORAL {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT PRODUCTOTEMPORALNOMBRE FROM SIGATG.PRODUCTOTEMPORAL WHERE EMPRESAID=10 AND PRODUCTOTEMPORALID IN (SELECT MAX(PRODUCTOTEMPORALID) FROM SIGATG.PRODUCTOTEMPORAL WHERE EMPRESAID=10 )"
