...
public class CRPREP_CLIENTE_CON_INTERNET_RESIDENCIAL__6MB_ALAJUELA {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT TRIM(MAX(CLIENTENRO)) FROM CONTRATO WHERE PRODUCTOID=1332 AND CONTRATOSTS='C' AND PLANCOMERCIALCLIENTEID=0 AND CLIENTENRO NOT IN (SELECT CLIENTENRO FROM CLIENTESERVICIO WHERE CLIENTESRVSTS='I') AND CLIENTENRO IN (SELECT CLIENTENRO FROM CLIENTE WHERE CIUDADID=2 AND CLIENTESTS NOT IN ('X','B','K') AND NEGOCIOSEGMENTO=1)"
