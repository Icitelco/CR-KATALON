...
public class CRPREP_ORDEN_SIN_APROVISIONAR {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT MAX(ORDENNRO) FROM ORDENSRV ORD WHERE ORDENSTS='E' AND ORDENTPO='I' AND CONTRATONRO IN (SELECT CONTRATONRO FROM CONTRATO CO WHERE ORD.EMPRESAID=CO.EMPRESAID AND CO.CONTRATONRO=ORD.CONTRATONRO AND PRODUCTOID IN (SELECT PRODUCTOID FROM PRODUCTO PO WHERE PRODUCTOAPROVISIONAR<>'N' AND CO.PRODUCTOID=PO.PRODUCTOID AND PO.EMPRESAID=CO.EMPRESAID))"
