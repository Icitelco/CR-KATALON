...
public class CRPERP_VALORES_CIERRE_CAJA {
	@Keyword
	def CRPREP_Cliente_Numero(String CAJANRO){
		...
		String query="SELECT SUM(RECIBOIMP) FROM SIGATG.RECIBO RE WHERE RECIBOSTS NOT IN ('X','I') AND RECIBOTPO='R' AND RECIBOFCH in (SELECT CAJADIAFCH FROM SIGATG.CAJADIA CJD WHERE CAJADIASTS='A' AND CAJANRO=".concat(CAJANRO).concat(" AND CJD.EMPRESAID=RE.EMPRESAID AND RE.CAJANRO=CJD.CAJANRO AND CJD.CAJADIAFCH=RE.RECIBOFCH)")
