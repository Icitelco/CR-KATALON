...
public class CRPERP_CLIENTE_FACTURAS_IMPAGAS {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT MAX(CLIENTENRO) FROM (SELECT COUNT(*) AS CANTFAC,CLIENTENRO FROM SIGATG.FACTURA WHERE FACTURASTS='I' AND CLIENTENRO IN (SELECT CLIENTENRO FROM SIGATG.CLIENTE WHERE GEODIV1ID=21 AND CLIENTESTS<>'X')GROUP BY CLIENTENRO)WHERE CANTFAC=3"
