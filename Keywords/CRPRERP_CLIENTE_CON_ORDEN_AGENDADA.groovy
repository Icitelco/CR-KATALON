...
public class CRPRERP_CLIENTE_CON_ORDEN_AGENDADA {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT TRIM(MAX(CLIENTENROORD)) FROM ORDENSRV WHERE ORDENSTS='A' AND ORDENTPO='R' AND CLIENTENROORD IN (SELECT CLIENTENRO FROM CLIENTE WHERE SUCURSALID=8 AND CLIENTESTS<>'X' AND NEGOCIOSEGMENTO=1)"
