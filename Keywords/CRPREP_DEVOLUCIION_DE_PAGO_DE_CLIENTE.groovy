...
public class CRPREP_DEVOLUCIION_DE_PAGO_DE_CLIENTE {
	@Keyword
	def CRPREP_Cliente_Numero(String Clientenro){
		...
		String query="SELECT MAX(RECIBONRO) FROM RECIBO RE WHERE RECIBONRO NOT IN (SELECT RECIBONRORBO FROM RECIBO RE2 WHERE RECIBOTPO='P' AND RE.CLIENTENRO=RE2.CLIENTENRO AND RE.EMPRESAID=RE2.EMPRESAID) and RECIBOTPO='R' AND RECIBOSTS='A' AND CLIENTENRO=".concat(Clientenro)
