...
public class CRPREP_CLIENTE_CON_INTERNET_RESIDENCIAL__12MB {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT MAX(CLIENTENRO) FROM CONTRATO CO WHERE PRODUCTOID=1461 AND CONTRATOSTS='C' AND PLANCOMERCIALCLIENTEID=0 AND NOT EXISTS (SELECT CLIENTESRVCONTRATO FROM CLIENTESERVICIO SV WHERE SV.CLIENTENRO=CO.CLIENTENRO AND SERVICIOID=5 AND CLIENTESRVSTS IN ('I', 'P', 'E'))"
