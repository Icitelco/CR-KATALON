...
public class CRPREP_USUARIO_ASIGNADO {
	@Keyword
	def CRPREP_Cliente_Numero(String cajaGrupo, String nombre){
		...
		String query ="SELECT MAX(CG.CAJAGRUPOID)CAJAID FROM SIGATG.CAJAGRUPOUSUARIO CG WHERE CG.CAJAGRUPOID IN (SELECT CU.CAJAGRUPOID FROM SIGATG.CAJAGRUPO CU WHERE CU.EMPRESAID = CG.EMPRESAID AND CAJAGRUPONOMBRE ='".concat(cajaGrupo) + "' AND CU.CAJAGRUPOID = CG.CAJAGRUPOID) AND CG.CAJAGRUPOUSUARIOID IN (SELECT CO.USUARIOID FROM SIGATG.COBRADOR CO WHERE CO.EMPRESAID = CG.EMPRESAID AND CO.USUARIOID = CG.CAJAGRUPOUSUARIOID AND COBRADORNOMBRE='".concat(nombre) + "' AND COBRADORSTS = 'A')"
		println(query);
