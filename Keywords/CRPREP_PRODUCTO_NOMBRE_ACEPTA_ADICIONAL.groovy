...
public class CRPREP_PRODUCTO_NOMBRE_ACEPTA_ADICIONAL {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT PRODUCTONOMBRE FROM SIGATG.PRODUCTO WHERE PRODUCTOID IN (SELECT MIN(PRODUCTOID) FROM SIGATG.PRODUCTO WHERE PRODUCTOCONADICIONAL=1 AND PRODUCTOSTS='A')"
