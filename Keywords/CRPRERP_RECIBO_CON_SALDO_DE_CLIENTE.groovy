...
public class CRPRERP_RECIBO_CON_SALDO_DE_CLIENTE {
	@Keyword
	def CRPREP_Cliente_Numero(String Clientenro){
		...
		String query="SELECT MAX (RECIBONRO) FROM RECIBO RE WHERE RECIBOTPO='R' AND RECIBOSTS='P' AND RECIBOSDO<>0 AND RECIBONRO NOT IN (SELECT RECIBONRORBO FROM RECIBO RE2 WHERE RECIBOTPO='P' AND RE.CLIENTENRO=RE2.CLIENTENRO AND RE.EMPRESAID=RE2.EMPRESAID) AND CLIENTENRO=".concat(Clientenro) 
