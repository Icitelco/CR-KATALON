...
public class CRPREP_CLIENTE_CON_PROPDUCTO_DE_PROMOCION {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT MAX(CLIENTENRO) FROM SIGATG.CONTRATO WHERE PRODUCTOID IN (SELECT PRODUCTOID FROM PRODUCTOPRM WHERE PROMOCIONID IN (SELECT MAX(PROMOCIONID) FROM SIGATG.PROMOCION))"
