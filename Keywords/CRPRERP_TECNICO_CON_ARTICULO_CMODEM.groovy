...
public class CRPRERP_TECNICO_CON_ARTICULO_CMODEM {
	@Keyword
	def CRPREP_Cliente_Numero(String pro){
		...
		String query="SELECT MAX(TECNICOID) FROM CENTROSTOCKTEC WHERE TECNICOID IN (SELECT TECNICOID FROM TECNICO WHERE TECNICOSTS='A') AND CENTROSTKID IN (SELECT CENTROSTKID FROM ARTICULOSERIE WHERE ARTICULOID IN (SELECT ARTICULOID FROM PRODUCTOMATERIALARTICULO WHERE PRODUCTOID=".concat(pro).concat(")").concat(")")
