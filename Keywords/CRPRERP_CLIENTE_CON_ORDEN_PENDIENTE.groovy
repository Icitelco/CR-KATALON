...
public class CRPRERP_CLIENTE_CON_ORDEN_PENDIENTE {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT TRIM(MAX(CLIENTENRO)) FROM CLIENTE WHERE SUCURSALID=8 AND CLIENTESTS='C' AND NEGOCIOSEGMENTO=1 AND CLIENTENRO NOT IN (SELECT CLIENTENROORD FROM ORDENSRV WHERE ORDENSTS IN ('P','A') AND ORDENTPO='R')"
