...
public class CRPRERP_CLIENTE_CON_PROMO {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT MAX(CO.CLIENTENRO)CLIENTE FROM CONTRATO CO WHERE CO.CONTRATONRO IN (SELECT MAX(CP.CONTRATONRO) FROM CONTRATOPROMOCION CP WHERE CP.CONTRATONRO = CO.CONTRATONRO AND CONTRATOPRMSTS = 'A') AND CO.CONTRATOSTS = 'C'"
