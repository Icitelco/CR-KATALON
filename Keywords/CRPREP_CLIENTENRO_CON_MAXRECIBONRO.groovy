...
public class CRPREP_CLIENTENRO_CON_MAXRECIBONRO {
	...
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT CLIENTENRO FROM SIGATG.RECIBO WHERE RECIBOTPO='R' AND RECIBOSTS='A' AND RECIBONRO IN (SELECT MAX(RECIBONRO) FROM SIGATG.RECIBO RE WHERE RECIBOTPO='R' AND RECIBOSTS='A')"
