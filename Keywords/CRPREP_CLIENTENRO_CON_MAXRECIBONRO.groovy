...
public class CRPREP_CLIENTENRO_CON_MAXRECIBONRO {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT CLIENTENRO FROM SIGATG.RECIBO WHERE RECIBOTPO='R' AND RECIBOSTS='A' AND RECIBONRO IN (SELECT MAX(RECIBONRO) FROM SIGATG.RECIBO RE WHERE RECIBOTPO='R' AND RECIBOSTS='A' AND RECIBONRO NOT IN (SELECT RECIBONRORBO FROM RECIBO RE2 WHERE RECIBOTPO='P' AND RE.CLIENTENRO=RE2.CLIENTENRO AND RE.EMPRESAID=RE2.EMPRESAID))"
