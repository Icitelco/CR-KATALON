...
public class CRPREP_CONTRATO_QUE_ADMITE_PROMOCION {
	@Keyword
	def CRPREP_Cliente_Numero(String Temp){
		...
		String query="SELECT MAX(CONTRATONRO)FROM SIGATG.CONTRATO CO WHERE CO.CONTRATONRO IN (SELECT CP.CONTRATONRO FROM CONTRATOPROMOCION CP WHERE CP.CONTRATONRO = CO.CONTRATONRO AND CONTRATOPRMSTS = 'A') AND CLIENTENRO=".concat(Temp)
