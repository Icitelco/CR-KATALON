...
public class CRPREP_FECHA_ORDEN_TECNICO_DISPATCHER {
	@Keyword
	def CRPREP_Cliente_Numero(){
		...
		String query="SELECT TO_CHAR(MAX(ORDENAGENDAFCH),'dd/mm/yyyy') FROM ORDENSRV WHERE ORDENSTS='E' AND TECNICOID=16600 AND ORDENAGENDANRO<>0 AND ORDENAGENDANRO IN (SELECT AGENDANRO FROM AGENDAREGISTRO WHERE AGENDAREGSTATUS='N' AND TECNICOID=16600)"
